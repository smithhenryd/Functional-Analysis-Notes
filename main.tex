...
\begin{note}
An immediate consequence of Theorem \ref{constructbasis} is that if $\text{span}\{v_1, \ldots, v_n\} = X$, then $\text{dim} \ X \leq n$. Also, if $\{v_1, \ldots, v_n\}$ is a linearly independent set, then $\text{dim} \ X \geq n$.
\end{note}
...
\begin{defn}[Null space]\label{nullspace}
The \textbf{null space} or \textbf{kernel} of a linear operator $T$ is the set of $x \in \mathscr{D}(T)$ such that $Tx = \0$. The null space is denoted by $\mathscr{N}(T)$.

From Definition \ref{linearoperator}, we remark that for every linear operator $T$, $\0 \in \mathscr{N}(T)$.
\end{defn}

\begin{note}
A quick remark on notation. Let $\mathscr{D}(T) \subset X$ and $\mathscr{R}(T) \subset Y$, where $X$ and $Y$ are vector spaces. We say
\begin{enumerate}
    \item $T: \mathscr{D}(T) \longrightarrow \mathscr{R}(T)$, $T$ is an operator from $\mathscr{D}(T)$ \textit{onto} $\mathscr{R}(T)$
    \item $T: \mathscr{D}(T) \longrightarrow Y$, $T$ is an operator from $\mathscr{D}(T)$ \textit{into} $Y$
    \item If $\mathscr{D}(T) = X$, $T: X \longrightarrow Y$
\end{enumerate}
\end{note}

\begin{examp}[Identity operator]
The \textbf{identity operator} $I_X: X \longrightarrow X$ is defined such that $I_X x := x$ for every $x \in X$. For shorthand, we write $I_X = I$. One should notice that $I$ is a bijection.
\end{examp}

\begin{examp}[Zero operator]
The \textbf{zero operator} $\0: X \longrightarrow Y$ is defined such that $\0x := \0_Y$ for every $x \in X$.
\end{examp}

\begin{examp}[Differentiation operator]
Let $X$ be the vector space consisting of all polynomials on $[a, b]$. We may define a linear operator by setting
\begin{align*}
    Tx := \frac{d}{dt}x(t), \qquad t \in [a,b]
\end{align*}
for every $x \in X$. This operator into onto $X$, but is not injective.
\end{examp}

\begin{examp}[Integration operator]
A linear operator $T$ from $C[a,b]$ into itself can be defined such that
\begin{align*}
    Tx := \int_a^t x(s) ds, \qquad t \in [a, b].
\end{align*}

$T$ is not onto $C[a,b]$, though. To see why this is the case, by the Fundamental Theorem of Calculus:
\begin{align*}
    \frac{d}{dt}Tx = x(t).
\end{align*}
That is, all $y \in \mathscr{R}(T)$ must have a continuous derivative. This is not true of all continuous functions. 
...
\begin{align*}
    ...
    \end{cases}
\end{align*}
defined on any interval $[a, b]$ containing $ t = 0$. $x(t)$ is everywhere differentiable on $\mathbb{R}$, including at $t = 0$, and its derivative is given by
\begin{align*}
    x'(t) = 
    \begin{cases}
    ...
    \end{cases}
\end{align*}
which is discontinuous at $t=0$. Thus, $x \notin \mathscr{R}(T)$, which means that $\mathscr{R}(T) \neq C[a,b]$.

\begin{figure}[H]
\begin{center}
\begin{tikzpicture}
\begin{axis}[axis lines=middle, scaled ticks=false, samples=1000, xlabel={$t$}, xticklabels={}, yticklabels={}]
\addplot[semithick][name path=f, domain=0.001:0.1] {x^2*sin(deg(1/x)))};
\addplot[semithick][name path=f1, domain=-0.1:-0.001] {x^2*sin(deg(1/x)))};
\end{axis}
\end{tikzpicture}
\end{center}
\caption*{$x(t)$ plotted on $[-0.1, 0.1]$}
\end{figure}
\end{examp}

\begin{examp}[Matrix multiplication]
Consider a field $K$. Let $A$ be an $m \times n$ matrix whose entries are elements of $K$. That is, we have
\begin{align*}
    A = 
    \begin{bmatrix}
    ...
    \end{bmatrix}
\end{align*}
where $(a_{i,j})_{i \in [m], j \in [n]} \subset K$. Then we can define an operator $T: K^n \longrightarrow K^m$ by
\begin{align*}
    Tx := Ax, \qquad x \in K^n.
\end{align*}
Here $K^n$ denotes the vector space consisting of all $n$-tuples whose entries are elements of $K$.
\end{examp}
...
\begin{lm}[Inverse of produce]\label{inversecomposition}
Let $T: X \longrightarrow Y$ and $S: Y \longrightarrow Z$ be bijective linear operators. Then the inverse $(ST)^{-1}$ of the product (i.e. the composition) $ST$ exists, and
\begin{align*}
    (ST)^{-1} = T^{-1}S^{-1}.
\end{align*}
\end{lm}
\begin{proof}
Since each of $T: X \longrightarrow Y$ and $S: Y \longrightarrow Z$ is bijective, then so is $ST: X \longrightarrow Z$. Thus, $(ST)^{-1}: Z \longrightarrow X$ exists. This implies that
\begin{align*}
    ST(ST)^{-1} = I_Z.
\end{align*}
Now, applying the map $S^{-1}$ to both sides of the equation,
\begin{align*}
    &(S^{-1}S)T(ST)^{-1} = S^{-1}I_Z\\
    \implies& T(ST)^{-1} = S^{-1}.
\end{align*}
Similarly, by applying $T^{-1}$ to both sides,
\begin{align*}
    &(T^{-1}T)(ST)^{-1} = T^{-1}S^{-1}\\
    \implies& (ST)^{-1} = T^{-1}S^{-1}.
\end{align*}
\end{proof}

\begin{note}
Lemma \ref{inversecomposition} is commonly used in the setting of matrix multiplication. Namely, for invertible matrices $A, B$ over the field $K$, we have $(AB)^{-1} = B^{-1}A^{-1}$.
\end{note}

\begin{exer}
Formulate condition (a) of Theorem \ref{inverseproperties} in terms of the null space of $T$.
...
\begin{exer}\label{invertiblepreserveindependence}
Let $T: \mathscr{D}(T) \longrightarrow Y$ be a linear operator whose inverse exists. If $\{x_1, \ldots, x_n\}$ is a linearly independent set in $\mathscr{D}(T)$, show that the set $\{Tx_1, \ldots, Tx_n\}$ is linearly independent in $Y$.
\begin{proof}
Suppose towards contradiction that $\{Tx_1, \ldots, Tx_n\}$ is linearly dependent. Then there exists scalars $a_1, \ldots, a_n \in K$ not all zero such
\begin{align*}
    \0_Y = a_1Tx_1 + \ldots + a_nTx_n
\end{align*}
By Theorem \ref{inverseproperties}, since $T^{-1}$ exists, then $T$ must satisfy $Tx_0 = \0_Y \implies x_0 = \0_X$. Therefore,
\begin{align*}
    &\0_Y = a_1Tx_1 + \ldots + a_nTx_n\\
    \implies& \0_Y = T(a_1x_1 + a_nx_n)\\
    \implies&\0_X = a_1x_1 + a_nx_n
\end{align*}
But this is a contradiction to the assumption that $\{x_1, \ldots, x_n\}$ is a linearly independent set in $\mathscr{D}(T)$. We conclude that $\{Tx_1, \ldots, Tx_n\}$ must be linearly independent in $Y$.
\end{proof}
\end{exer}
...
\begin{enumerate}[(a)]
    ...
    Finally, we must prove that the triangle inequality holds. In a similar fashion to as in the previous axiom, we use the triangle inequality for the norm on $Y$:
    \begin{align*}
        \|S + T\| &= \sup_{x \in X\setminus\{\0_X\}} \frac{\|(S +T)x\|}{\|x\|} = \sup_{x \in X\setminus\{\0_X\}} \frac{\|Sx + Tx\|}{\|x\|} \leq \sup_{x \in X\setminus\{\0_X\}} \frac{\|Sx\| + \|Tx\|}{\|x\|}\\
        &= \sup_{x \in X\setminus\{\0_X\}} \left( \frac{\|Sx\|}{\|x\|} + \frac{\|Tx\|}{\|x\|} \right) \leq \sup_{x \in X\setminus\{\0_X\}} \frac{\|Sx\|}{\|x\|} + \sup_{x \in X\setminus\{\0_X\}} \frac{\|Tx\|}{\|x\|}\\
        &= \|S\| + \|T\|.
    \end{align*}
\end{enumerate}
\end{proof}
